    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-apple-gray-500 mt-1">Manage payment methods</p>
            </div>
                    ...
                    @forelse($paymentMethods as $paymentMethod)
                        <tr class="hover:bg-apple-gray-50 transition-colors duration-150">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-apple-gray-900">{{ $paymentMethod->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-apple-gray-500">{{ $paymentMethod->complaints_count }} tickets</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('payment-methods.edit', $paymentMethod) }}" class="text-apple-blue hover:text-apple-blue-dark mr-4">Edit</a>
                                <form action="{{ route('payment-methods.destroy', $paymentMethod) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
